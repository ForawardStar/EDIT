...
from models import *
from datasets_CrossDomain import *
from utils import *
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        ref = Variable(batch['ref'].type(Tensor))
        ...
        ref_mean_std = mean_std(ref_features)

        weights_AB_recov = metanet(realB_mean_std, label_AB)
        weights_BA_recov = metanet(realA_mean_std, label_BA)
        weights_AB = metanet(ref_mean_std, label_AB)
        weights_BA = metanet(ref_mean_std, label_BA)
        ...
        fake_A = G(real_B)
        G.set_weights(weights_BA_recov, 0)
        recov_A = G(fake_B)
        G.set_weights(weights_AB_recov, 0)
        recov_B = G(fake_A)
        ...
        loss_GAN_AB = criterion_GAN(D_B(fake_B, label_AB), valid)
        #fake_A = G_BA(real_B,label)
        loss_GAN_BA = criterion_GAN(D_A(fake_A, label_BA), valid)
        ...
        # Real loss
        loss_real = criterion_GAN(D_A(real_A,label_BA), valid)
        # Fake loss (on batch of previously generated samples)
        fake_A_, label_ = fake_A_buffer.push_and_pop(fake_A, label_BA)

        loss_fake = criterion_GAN(D_A(fake_A_.detach(), label_), fake)
        # Total loss
        ...
        # Real loss
        loss_real = criterion_GAN(D_B(real_B,label_AB), valid)
        # Fake loss (on batch of previously generated samples)
        fake_B_,label_ = fake_B_buffer.push_and_pop(fake_B, label_AB)
        loss_fake = criterion_GAN(D_B(fake_B_.detach(), label_), fake)
        # Total loss
        ...
        del valid , fake ,real_A, real_B, ref_features, real_A_features , real_B_features ,  weights_AB , weights_BA
    ...
    if opt.checkpoint_interval != -1 and epoch % opt.checkpoint_interval == 0:
        ...
        torch.save(D_A, 'saved_models/%s/D_A_%d.pth' % (opt.dataset_name, epoch))
